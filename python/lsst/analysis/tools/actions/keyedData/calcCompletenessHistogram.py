...
from ..config import MagnitudeBinnedMetricsConfig
                ...
                if completeness:
                    if idx_percentile > 0:
                        if completeness < percentiles[idx_percentile - 1]:
                            idx_percentile -= 1
                    if (idx_percentile < n_percentiles) and (completeness > percentiles[idx_percentile]):
                        percentile_completeness[percentiles[idx_percentile]] = (maximum + minimum) / 2.0
                        idx_percentile += 1
