...
class HistPlot(PlotAction):
    ...
    def makePlot(
        ...
        for panel, ax in zip(self.panels, axs):
            ...
            nums, meds, mads, stats_dict = self._makePanel(
                ...
                ncols=ncols,
                addThreshold=self.panels[panel].addThreshold,
            )
    ...
    def _makePanel(
        self, data, panel, ax, colors, label_font_size=9, legend_font_size=7, ncols=1, addThreshold=False
    ):
        ...
        if self.panels[panel].addThreshold:
