    ...
    def __init__(self, *, config: AnalysisBaseConfig = None):  # type: ignore
        ...
        for name, action in names.items():
            ...
            if action.parameterizedBand and "band" not in self.dimensions:
                ...
                # in the output names, then it is likely a user error. To avoid
                # the band from appearing, either parametrizedBand needs to be
                # set to False or the bands should be an empty list.
                multiple = True
                dimensions = self.dimensions.union({"band"})
                # Struct attribute has the structure of
...
class AnalysisPipelineTask(PipelineTask):
    ...
    def putByBand(self, butlerQC, outputs, outputRefs):
