...
class CalexpAnalysisTask(AnalysisPipelineTask):
    ...
    def runQuantum(
        ...
        planesDict = {
            "image": inputs["data"].image.getArray(),
            "mask": inputs["data"].mask.getArray(),
            "variance": inputs["data"].variance.getArray(),
        }
        maskPlaneDict = inputs["data"].mask.getMaskPlaneDict()

        outputs = self.run(data=planesDict, maskPlaneDict=maskPlaneDict)
