...
class MakeDiffimKernelQuiverPlotVisitTask(pipeBase.PipelineTask):
    ...
    def run(self, spatiallySampledMetrics, visit_summary):
        ...
        for data_ref in spatiallySampledMetrics:
            ...
            data = data_ref.get()
            dataSelector = np.isfinite(data["psfMatchingKernel_direction"]) & \
                np.isfinite(data["psfMatchingKernel_length"])
            ...
            draw_detector_outlines(detector, visit_summary, color='red', ax=ax, linewidth=0.5)
