...
class PerTractPropertyMapAnalysisTaskTestCase(lsst.utils.tests.TestCase):
    ...
    @staticmethod
    def _isHistogramAxes(ax, binsCount, legendLabels, errors):
    ...
    @staticmethod
    def _isColorbarAxes(ax):
        return any(child.__class__.__name__ == "_ColorbarSpine" for child in ax.get_children())
    ...
    def _validateFigureStructure(self, fig, atool, binsCount, xlabel, zoomFactors):
        ...
        totalColorbarAxes = sum(1 for ax in axes if self._isColorbarAxes(ax))
